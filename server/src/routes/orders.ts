...
router.get('/', async (req, res) => {
  try {
    ...
    const totalCount = await OrderModel.findByUserId(user.id, 1000, 0, 'created_at', 'DESC');
    const totalPages = Math.ceil(totalCount.length / parseInt(limit as string));
...
router.post('/create', async (req, res) => {
  try {
    ...
    // Get user's cart
    const cart = await CartModel.findByUserId(user.id);
    if (!cart) {
    ...
    for (const item of orderItems) {
      ...
      await ProductModel.updateStock(item.product_id!, item.quantity, order.id);
    }
    ...
    try {
      await cartAbandonmentService.markCartRecovered(user.id);
    } catch (error) {
    ...
    try {
      await recommendationService.updatePurchasePatterns(order.id);
    } catch (error) {
...
router.put('/:orderId/shipping', async (req, res) => {
  try {
    ...
    const updatedOrder = await OrderModel.updateShippingStatus(orderId, shipping_status, tracking_number);
...
router.post('/:orderId/cancel', async (req, res) => {
  try {
    ...
    let refundAmount = 0;
    let refundStatus: 'none' | 'pending' | 'partial' | 'completed' | 'failed' = 'none';
    ...
    if (order.awb_number) {
      try {
        if (isShipped) {
          ...
          const returnResponse = await shiprocketService.createReturn(returnData);
          
          if (returnResponse.return_id) {
            // Update order with return information
            await OrderModel.initiateReturn(orderId, returnResponse.return_id.toString(), cancellation_reason);
            console.log('Shiprocket return initiated:', returnResponse.return_id);
          }
        } else {
          // For non-shipped orders, cancel normally
          await shiprocketService.cancelShipment([order.awb_number]);
    ...
    const cancelledOrder = await OrderModel.updateCancellation(
      orderId,
      cancellation_reason,
    ...
    try {
      await emailService.sendOrderCancellationEmail(cancelledOrder || order, user);
...
router.get('/:orderId/tracking', async (req, res) => {
  try {
    ...
    if (order.awb_number) {
      try {
        trackingInfo = await shiprocketService.trackByAWB(order.awb_number);
      } catch (trackingError) {
...
router.put('/:orderId/refund-status', async (req, res) => {
  try {
    ...
    const updatedOrder = await OrderModel.updateRefundStatus(orderId, refund_status);
