...
const validateEmailChange = [
    (0, express_validator_1.body)('newEmail').isEmail().normalizeEmail().withMessage('Valid email is required'),
    (0, express_validator_1.body)('password').notEmpty().withMessage('Password is required for email change'),
    validation_1.handleValidation
];
// Get user profile
...
// Change email
router.put('/email', validateEmailChange, async (req, res) => {
    try {
        const userId = req.user.id;
        const { newEmail, password } = req.body;
        // Get current user data
        const userResult = await database_1.default.query('SELECT email, password_hash FROM users WHERE id = $1', [userId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'User not found'
        ...
        const existingUser = await database_1.default.query('SELECT id FROM users WHERE email = $1 AND id != $2', [newEmail, userId]);
...
router.delete('/account', [
    ...
    (0, express_validator_1.body)('confirmDelete').equals('DELETE').withMessage('Confirmation must be "DELETE"'),
    validation_1.handleValidation
], async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        // Get user data
        const userResult = await database_1.default.query('SELECT email, name, password_hash FROM users WHERE id = $1', [userId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'User not found'
    ...
    try {
      ...
      `, [userId]);
        ...
        if (type === 'all' || type === 'reviews') {
            const reviewResult = await database_1.default.query(`
        SELECT 
          'review' as type,
          r.id,
          p.name as reference,
