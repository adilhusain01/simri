...
router.post('/register', [
    ...
    (0, express_validator_1.body)('phone').optional().matches(/^[\+]?[1-9][\d]{0,15}$/).withMessage('Invalid phone number format'),
...
router.post('/forgot-password', [
    ...
    try {
        ...
        // Rate limiting check
        const recentAttempts = await authService_1.authService.getRecentResetAttempts(email);
        if (recentAttempts >= 3) {
        ...
        const result = await authService_1.authService.generatePasswordResetToken(email);
        res.json({
...
router.get('/reset-password/:token', async (req, res) => {
    try {
        const { token } = req.params;
        const verification = await authService_1.authService.verifyPasswordResetToken(token);
...
router.post('/reset-password', [
    ...
    try {
        ...
        const passwordValidation = authService_1.authService.validatePasswordStrength(newPassword);
...
router.post('/validate-password', [
    ...
    try {
        ...
        const validation = authService_1.authService.validatePasswordStrength(password);
        res.json({
            success: true,
            data: {
