...
// ========================
// Create new product
router.post('/products', validation_1.validateProduct, async (req, res) => {
    try {
...
router.put('/products/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const productData = req.body;
        const updatedProduct = await Product_1.ProductModel.update(id, productData);
        if (!updatedProduct) {
...
router.delete('/products/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const deleted = await Product_1.ProductModel.delete(id);
        if (!deleted) {
            return res.status(404).json({ success: false, message: 'Product not found' });
        }
...
router.get('/products', async (req, res) => {
    try {
        const { limit = 50, offset = 0, status, category, search, sort_by = 'created_at', sort_order = 'desc' } = req.query;
        let query = `
...
router.get('/orders', async (req, res) => {
    try {
        const { limit = 50, offset = 0, status, payment_status, shipping_status, start_date, end_date, user_email, search, sort_by = 'created_at', sort_order = 'desc' } = req.query;
        let query = `
...
router.patch('/orders/:id/status', async (req, res) => {
    try {
        ...
        const updatedOrder = await Order_1.OrderModel.updateStatus(id, status);
        if (!updatedOrder) {
            return res.status(404).json({
                success: false,
                message: 'Order not found'
...
router.patch('/orders/:id/payment-status', async (req, res) => {
    try {
        ...
        const updatedOrder = await Order_1.OrderModel.updatePaymentStatus(id, payment_status, payment_id);
        if (!updatedOrder) {
            return res.status(404).json({
                success: false,
                message: 'Order not found'
...
router.patch('/orders/:id/shipping-status', async (req, res) => {
    try {
        ...
        const updatedOrder = await Order_1.OrderModel.updateShippingStatus(id, shipping_status, tracking_number);
        if (!updatedOrder) {
            return res.status(404).json({
                success: false,
                message: 'Order not found'
...
router.patch('/orders/:id/cancel', async (req, res) => {
    try {
        ...
        const updatedOrder = await Order_1.OrderModel.updateCancellation(id, cancellation_reason, refund_amount);
        if (!updatedOrder) {
            return res.status(404).json({
                success: false,
                message: 'Order not found'
        ...
        for (const orderId of order_ids) {
            try {
                ...
                switch (action) {
                    case 'update_status':
                        if (status) {
                            updatedOrder = await Order_1.OrderModel.updateStatus(orderId, status);
                        }
                        break;
                    case 'update_shipping':
                        if (shipping_status) {
                            updatedOrder = await Order_1.OrderModel.updateShippingStatus(orderId, shipping_status);
                        }
                        break;
                    case 'update_payment':
                        if (payment_status) {
                            updatedOrder = await Order_1.OrderModel.updatePaymentStatus(orderId, payment_status);
                        }
                        break;
                    default:
...
router.get('/orders/export', async (req, res) => {
    try {
        const { format = 'csv', status, payment_status, start_date, end_date } = req.query;
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        const { period = '30', startDate, endDate, comparisonStartDate, comparisonEndDate, includeComparison = 'false', category_id, 
        // Legacy support
        start_date, end_date, compare_period = 'false' } = req.query;
        ...
        if (actualStartDate && actualEndDate) {
            ...
            if (shouldCompare && actualComparisonStartDate && actualComparisonEndDate) {
                ...
                const daysDiff = Math.ceil((new Date(actualEndDate).getTime() - new Date(actualStartDate).getTime()) / (1000 * 60 * 60 * 24));
                const compareStartDate = new Date(new Date(actualStartDate).getTime() - (daysDiff * 24 * 60 * 60 * 1000));
                const compareEndDate = new Date(actualStartDate);
                compareDateCondition = `created_at >= $3::timestamp AND created_at <= $4::timestamp`;
    ...
    `;
        ...
        const revenueResult = await database_1.default.query(revenueQuery, params.slice(0, useDateRange ? 2 : 0));
      ...
      `;
            ...
            const compareResult = await database_1.default.query(compareQuery, compareParams);
    ...
    `;
        ...
        const customerResult = await database_1.default.query(customerQuery, params.slice(0, useDateRange ? 2 : 0));
      ...
      `;
            ...
            const compareCustomerResult = await database_1.default.query(compareCustomerQuery, compareCustomerParams);
    ...
    `;
      ...
      LIMIT 12
    `;
        const productResult = await database_1.default.query(productQuery, productParams);
      ...
      LIMIT 10
    `;
        const categoryResult = await database_1.default.query(categoryQuery, params.slice(0, useDateRange ? 2 : 0));
    ...
    `;
        const marketingResult = await database_1.default.query(marketingQuery, params.slice(0, useDateRange ? 2 : 0));
      ...
      WHERE ${dateCondition}
    `;
        const funnelResult = await database_1.default.query(funnelQuery, params.slice(0, useDateRange ? 2 : 0));
      ...
      ORDER BY date ASC
    `;
        const trendsResult = await database_1.default.query(trendsQuery, params.slice(0, useDateRange ? 2 : 0));
      ...
      `;
        ...
        const statusResult = await database_1.default.query(statusQuery, statusParams);
        // 9. CART ABANDONMENT INSIGHTS
        let abandonmentQuery = '';
        let abandonmentParams = [];
        if (useDateRange) {
            abandonmentQuery = `
        SELECT 
      ...
      `;
        ...
        const abandonmentResult = await database_1.default.query(abandonmentQuery, abandonmentParams);
        ...
        res.json({
            ...
            data: {
                revenue: {
                    ...
                    revenue_growth: calculateGrowth(parseFloat(current.total_revenue || 0), parseFloat(comparison.total_revenue || 0)),
                    total_orders: parseInt(current.total_orders || 0),
                    previous_period_orders: parseInt(comparison.total_orders || 0),
                    orders_growth: calculateGrowth(parseInt(current.total_orders || 0), parseInt(comparison.total_orders || 0)),
                    avg_order_value: parseFloat(current.average_order_value || 0),
                    previous_period_aov: parseFloat(comparison.average_order_value || 0),
                    aov_growth: calculateGrowth(parseFloat(current.average_order_value || 0), parseFloat(comparison.average_order_value || 0)),
                ...
                customers: {
                    ...
                    customer_growth: calculateGrowth(parseInt(customer.total_customers || 0), parseInt(customerComparison.total_customers || 0)),
...
router.delete('/categories/:id', async (req, res) => {
    try {
        ...
        const categoryResult = await database_1.default.query('SELECT image_url FROM categories WHERE id = $1', [id]);
        const categoryImageUrl = categoryResult.rows[0]?.image_url;
        // Check how many products will be affected
        const affectedProductsResult = await database_1.default.query('SELECT COUNT(*) as count FROM products WHERE category_id = $1', [id]);
        ...
        const affectedSubcategoriesResult = await database_1.default.query('SELECT COUNT(*) as count FROM categories WHERE parent_id = $1', [id]);
        const affectedSubcategoriesCount = parseInt(affectedSubcategoriesResult.rows[0].count);
        // Get subcategory images for deletion
        const subcategoriesResult = await database_1.default.query('SELECT image_url FROM categories WHERE parent_id = $1 AND image_url IS NOT NULL', [id]);
        ...
        if (categoryImageUrl) {
            try {
                const { uploadService } = await Promise.resolve().then(() => __importStar(require('../services/uploadService')));
...
router.get('/users', async (req, res) => {
    try {
        const { limit = 50, offset = 0, role, search, sort_by = 'created_at', sort_order = 'desc' } = req.query;
...
router.get('/analytics/export', async (req, res) => {
    try {
        const { startDate, endDate, comparisonStartDate, comparisonEndDate, includeComparison = 'false', format = 'csv' } = req.query;
        ...
        const actualStartDate = formatDate(startDate);
