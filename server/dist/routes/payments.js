...
router.post('/create-order', async (req, res) => {
    try {
        ...
        // Get order details
        const order = await Order_1.OrderModel.findById(order_id);
        if (!order) {
        ...
        await Order_1.OrderModel.updateRazorpayOrderId(order.id, razorpayOrder.id);
        res.json({
            success: true,
            data: {
...
router.post('/verify', async (req, res) => {
    try {
        ...
        const updatedOrder = await Order_1.OrderModel.updatePaymentComplete(order_id, 'paid', razorpay_payment_id);
        // Also update order status to confirmed
        await Order_1.OrderModel.updateStatus(order_id, 'confirmed');
        // Automatically create Shiprocket shipping after payment confirmation
        try {
            const orderItems = await Order_1.OrderItemModel.findByOrderId(order_id);
            if (orderItems.length > 0) {
                ...
                if (shiprocketResponse.order_id) {
                    ...
                    await Order_1.OrderModel.updateShiprocketInfo(order_id, shiprocketResponse.order_id.toString(), shiprocketResponse.shipment_id?.toString(), shiprocketResponse.awb_code, shiprocketResponse.courier_name);
                    // Update shipping status to processing
                    await Order_1.OrderModel.updateShippingStatus(order_id, 'processing');
        ...
        try {
            await emailService_1.emailService.sendOrderConfirmationEmail(updatedOrder || order, user);
