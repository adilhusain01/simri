...
class CouponService {
    ...
    async validateCoupon(code, orderAmount, userId) {
        try {
            // Get coupon details
            const couponResult = await database_1.default.query(`
        SELECT * FROM coupons 
      ...
      `, [code]);
            ...
            if (coupon.usage_limit && userId) {
                const userUsageResult = await database_1.default.query(`
    ...
    async getActiveCoupons(isPublic = false) {
        try {
            let query = `
        SELECT * FROM coupons 
    ...
    async createCoupon(couponData, _adminId) {
        try {
            const { code, name, description, type, value, minimum_order_amount, maximum_discount_amount, usage_limit, valid_from, valid_until } = couponData;
            ...
            const existingCoupon = await database_1.default.query('SELECT id FROM coupons WHERE UPPER(code) = UPPER($1)', [code]);
    ...
    async getBestCouponForOrder(orderAmount, userId) {
        try {
