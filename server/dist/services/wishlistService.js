...
class WishlistService {
    ...
    async addToWishlist(userId, productId) {
        try {
            ...
            const productCheck = await database_1.default.query('SELECT id FROM products WHERE id = $1 AND is_active = true', [productId]);
            if (productCheck.rows.length === 0) {
                return { success: false, message: 'Product not found' };
            }
            // Check if already in wishlist
            const existingItem = await database_1.default.query('SELECT id FROM wishlists WHERE user_id = $1 AND product_id = $2', [userId, productId]);
    ...
    async isInWishlist(userId, productId) {
        try {
            const result = await database_1.default.query('SELECT id FROM wishlists WHERE user_id = $1 AND product_id = $2', [userId, productId]);
    ...
    async moveItemToCart(userId, itemId, quantity = 1) {
      ...
      `, [itemId, userId]);
            ...
            const cartItemCheck = await client.query('SELECT id, quantity FROM cart_items WHERE cart_id = $1 AND product_id = $2', [cartId, item.product_id]);
            let cartItem;
            if (cartItemCheck.rows.length > 0) {
                // Update quantity if already in cart
                const updateResult = await client.query('UPDATE cart_items SET quantity = quantity + $1, updated_at = NOW() WHERE id = $2 RETURNING *', [quantity, cartItemCheck.rows[0].id]);
        ...
        `, [cartId, item.product_id, quantity, item.price]);
            ...
            await client.query('DELETE FROM wishlists WHERE id = $1', [itemId]);
        ...
        try {
            ...
            for (const productId of productIds) {
                try {
                    ...
                    const wishlistCheck = await client.query('SELECT id FROM wishlists WHERE user_id = $1 AND product_id = $2', [userId, productId]);
                    if (wishlistCheck.rows.length === 0) {
                        errors.push(`Product ${productId} not found in wishlist`);
                        continue;
                    }
                    // Get product details
                    const productResult = await client.query('SELECT price, stock_quantity FROM products WHERE id = $1 AND is_active = true', [productId]);
                    ...
                    // Check if already in cart
                    const cartItemCheck = await client.query('SELECT id, quantity FROM cart_items WHERE cart_id = $1 AND product_id = $2', [cartId, productId]);
                    if (cartItemCheck.rows.length > 0) {
                        // Update quantity if already in cart
                        await client.query('UPDATE cart_items SET quantity = quantity + 1, updated_at = NOW() WHERE id = $1', [cartItemCheck.rows[0].id]);
                    }
                    else {
                        // Add new item to cart
                        await client.query(`
            ...
            `, [cartId, productId, product.price]);
                    ...
                    // Remove from wishlist
                    await client.query('DELETE FROM wishlists WHERE user_id = $1 AND product_id = $2', [userId, productId]);
                    movedCount++;
                }
                catch (itemError) {
    ...
    async getWishlistInsights(userId) {
        try {
            const result = await database_1.default.query(`
        SELECT 
